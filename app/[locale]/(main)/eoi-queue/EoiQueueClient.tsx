...
export function EoiQueueClient({ rows: allRows }: { rows: OccupationStatRow[] }) {
  ...
  const rows = useMemo(
    () => allRows.filter((row) => category === "ALL" || row.tier === category),
    [allRows, category]
  );
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums text-slate-700">{row.cutoff189 ?? "—"}</td>
                    <td className="px-4 py-3">
