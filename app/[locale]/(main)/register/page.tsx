  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        ...
        setSuccess(true);
        // Sign in automatically then redirect
      ...
      <div className="w-full max-w-sm">
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <div className="mb-6 text-center">
            ...
            <h1 className="mt-2 text-lg font-bold text-slate-900">Create your account</h1>
            <p className="mt-1 text-sm text-slate-500">Save results and track your visa journey</p>
          </div>
          ...
          <button
            ...
            disabled={isPending}
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 hover:border-slate-300 disabled:opacity-60"
          >
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="confirm" className="mb-1.5 block text-xs font-medium text-slate-700">Confirm Password</label>
                <div className="relative">
                  <input
              ...
              {error && (
                <p className="rounded-lg bg-rose-50 border border-rose-200 px-3 py-2 text-xs text-rose-600">{error}</p>
              )}
          ...
          <p className="mt-5 text-center text-xs text-slate-500">
            ...
            <Link href={`/${locale}/sign-in`} className="font-medium text-indigo-600 hover:underline">
              Sign in
            </Link>
          </p>
        </div>
