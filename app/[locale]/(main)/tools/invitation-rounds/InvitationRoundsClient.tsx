...
const SUBCLASS_BG: Record<string, string> = {
  "189": "bg-[#53917E]/10 text-[#53917E]",
  "190": "bg-emerald-500/10 text-emerald-700",
  "491": "bg-amber-500/10 text-amber-700",
};
    ...
    <div className="rounded-2xl border border-slate-200 bg-card p-5 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1.5 text-xs text-slate-600">{visaName}</p>
        </div>
      ...
      {latest ? (
        <>
          <p className="mt-3 text-3xl font-bold text-slate-900">
            ...
            <span className="ml-1 text-base font-normal text-slate-600">{t("ir.pts")}</span>
          </p>
          <p className="mt-0.5 text-xs text-slate-600">{formatDate(latest.date)}</p>
          {delta !== 0 && (
            <p
        ...
        <p className="mt-3 text-sm text-slate-600">{t("ir.noData")}</p>
      )}
    </div>
  );
}
...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-card p-3 shadow-lg text-xs">
      <p className="mb-1.5 font-semibold text-slate-600">
  ...
  return (
    <div className="space-y-8 pt-20">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-card p-1 shadow-sm">
      ...
      {/* Chart */}
      <div className="rounded-2xl border border-slate-200 bg-card p-5 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold text-slate-900">{t("ir.pointsCutoffTrend", "Points Cutoff Trend")}</h2>
        {chartData.length === 0 ? (
      ...
      <div className="rounded-2xl border border-slate-200 bg-card shadow-sm overflow-hidden">
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-sm font-semibold text-slate-900">{t("ir.invitationRounds", "Invitation Rounds")}</h2>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {shown.map((r, i) => (
                    <tr
                      key={r.id}
                      className={`border-b border-slate-200 transition hover:bg-[#53917E]/10 ${
                        i % 2 === 1 ? "bg-card" : "bg-[#53917E]/5"
                      }`}
                    >
            ...
            {hasMore && (
              <div className="border-t border-slate-200 px-5 py-4 text-center">
                <button
                  ...
                  className="rounded-lg border border-slate-200 bg-card px-5 py-2 text-sm font-medium text-slate-600 transition hover:bg-[#53917E]/10"
                >
      ...
      <div className="rounded-2xl border border-slate-200 bg-card shadow-sm overflow-hidden">
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-sm font-semibold text-slate-900">{t("ir.pointsByOccupation", "Points by Occupation")}</h2>
          <p className="mt-0.5 text-xs text-slate-600">
          ...
          <div className="mt-3">
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-[#53917E] focus:ring-2 focus:ring-[#53917E]/20"
            />
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredOccupationPoints.map((row, i) => (
                <tr
                  key={row.occupation}
                  className={`border-b border-slate-200 ${i % 2 === 1 ? "bg-card" : "bg-[#53917E]/5"}`}
                >
                  <td className="px-4 py-3 text-slate-600">{row.occupation}</td>
                  <td className="px-4 py-3 text-right font-semibold text-slate-900">
                    {row.subclass189 ?? "—"}
                  </td>
                  <td className="px-4 py-3 text-right font-semibold text-slate-900">
      ...
      <div className="rounded-2xl border border-[#53917E]/20 bg-[#53917E]/10 p-6 shadow-sm">
        ...
        {alertMsg ? (
          <div className="mt-5 flex items-start gap-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 px-4 py-3">
          ...
          <form onSubmit={handleAlert} className="mt-5 grid gap-3 sm:grid-cols-4">
            ...
            {alertError && (
              <div className="sm:col-span-4">
                <p className="rounded-lg bg-rose-500/10 border border-rose-500/30 px-3 py-2 text-xs text-rose-700">
                  {alertError}
                </p>
              </div>
            )}
