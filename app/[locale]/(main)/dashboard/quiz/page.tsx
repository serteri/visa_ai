...
function readinessBadge(level: string | null) {
  if (level === "high") return "border-emerald-500/30 bg-emerald-500/10 text-emerald-700";
  if (level === "medium") return "border-amber-500/30 bg-amber-500/10 text-amber-700";
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Your saved quiz results and recommendations.</p>
        </div>
      ...
      {latest && latestRecs.length > 0 && (
        <Card className="border-violet-500/30 bg-violet-500/10">
          <CardHeader>
            <CardTitle className="text-sm text-violet-700">Top recommendations from your latest result</CardTitle>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2">
              {latestRecs.slice(0, 3).map((rec, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-violet-700">
                  <span className="mt-0.5 h-4 w-4 shrink-0 rounded-full bg-violet-500/30 text-center text-[10px] font-bold leading-4 text-violet-700">{i + 1}</span>
                  {rec}
                </li>
              ))}
            </ul>
      ...
      {results.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
            ...
            <div>
              <p className="font-semibold text-slate-900">No quiz results yet</p>
              <p className="mt-1 text-sm text-slate-600">Take the PR readiness quiz and your result will be saved here.</p>
            </div>
            ...
            <Card key={result.id}>
              <CardContent className="flex flex-wrap items-center justify-between gap-4 p-5">
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-violet-500/10 border border-violet-500/30 text-lg font-bold text-violet-700">
                    {result.score ?? "—"}%
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-slate-900">Quiz result {results.length - idx}</p>
                      {idx === 0 && <Badge className="bg-violet-500/20 text-violet-700 text-xs">Latest</Badge>}
                    </div>
                ...
                <form action={async () => { "use server"; await deleteQuizResult(result.id); }}>
                  <button type="submit" className="rounded-lg border border-slate-200 p-2 text-slate-600 transition-colors hover:border-rose-500/40 hover:text-rose-400" aria-label="Delete">
