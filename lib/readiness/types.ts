...
export type ReadinessReport = {
  ...
  /**
   * Narrative/advisory layer generated by lib/ai/generate-premium-strategy.ts
   * (RAG + LLM structured output), attached after the deterministic report
   * above is fully computed. Never used as a source of truth for points or
   * eligibility -- see that module's system prompt. Absent when the AI call
   * failed or wasn't attempted (e.g. missing API key), so consumers must
